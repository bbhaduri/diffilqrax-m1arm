...
from .lqr import (
    ...
    ModelDims,
    kkt,
    solve_lqr, solve_lqr_swap_x0, symmetrise_tensor, bmm
)
...
def get_qra_bar(dims: ModelDims,params: Params, tau_bar: Array, tau_bar_f: Array) -> Tuple[Array, Array, Array]:
    ...
    return q_bar, jnp.concatenate([r_bar, jnp.zeros(shape=(1,dims.m,))], axis = 0), a_bar
...
def dlqr(dims: ModelDims, params: Params, tau_guess: Array) -> Tuple[Array, Array, Array, Array]:
    ...
    tau_star =  jnp.concatenate([Xs_star[:,...],  jnp.concatenate([Us_star, jnp.zeros(shape=(1,dims.m))])], axis = 1)
    return tau_star 
...
def fwd_dlqr(dims: ModelDims, params: Params, tau_guess: Array):
    ...
    new_lqr = LQR(A=lqr.A, B=lqr.B, a=jnp.zeros_like(lqr.a), 
                        ...
                        Qf=lqr.Qf, qf=lqr.qf - mm(lqr.Qf, Xs_star[-1]), 
...
def rev_dlqr(dims: ModelDims, res, tau_bar) -> Params:
  ...
  """
    ...
    tau_star = jnp.concatenate([Xs_star, jnp.concatenate([Us_star, jnp.zeros(shape=(1,M))], axis = 0)], axis=1)
    n = dims.n 
